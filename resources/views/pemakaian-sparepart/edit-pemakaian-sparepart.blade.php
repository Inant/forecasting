                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header card-header-success">
                            <h4 class="card-title ">Edit Pemakaian Bahan Penunjang</h4>
                            {{-- <p class="card-category">Semua User</p> --}}
                        </div>
                        <div class="card-body">
                                <form method="POST" action="{{ route('pemakaian-sparepart.update', $pemakaianSparepart->id) }}">
                                    @csrf
                                    @method('PUT')
                                    <div class="row mt-3">
                                            ...
                                            has-error @enderror">
                                                ...
                                                <select name="bulan" id="bulan" class="form-control" required>
                                                    ...
                                                    @foreach ($bulan as $item)
                                                        <option value="{{$item['bulan']}}" style="color: black" {{old('bulan', $pemakaianSparepart->bulan) == $item['bulan'] ? 'selected' : '' }}>{{$item['nama']}}</option>
                                                    @endforeach
                                                </select>
                                            ...
                                            has-error @enderror">
                                                ...
                                                <select name="tahun" id="tahun" class="form-control" required>
                                                    ...
                                                    @for ($y = 2018; $y <= date('Y'); $y++)
                                                        <option value="{{$y}}" style="color: black" {{old('tahun', $pemakaianSparepart->tahun) == $y ? 'selected' : '' }}>{{$y}}</option>
                                                    @endfor
                                                </select>
                                                <span class="material-icons form-control-feedback">clear</span>
                                                @error('tahun')
